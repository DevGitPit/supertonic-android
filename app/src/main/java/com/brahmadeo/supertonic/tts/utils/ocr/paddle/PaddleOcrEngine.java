...
import com.brahmadeo.supertonic.tts.utils.ocr.OcrEngine;
import com.brahmadeo.supertonic.tts.utils.ocr.OcrResult;
...
public final class PaddleOcrEngine implements OcrEngine {
    ...
    public boolean isAvailable(Context ctx) {
        try {
            // Verfügbar, wenn Modelle in Assets vorhanden sind.
            return PaddleAssets.areModelsPresent(ctx, "en") || PaddleAssets.areModelsPresent(ctx, "hi");
        } catch (Throwable t) {
    ...
    private static boolean assetExists(Context ctx, String name) {
        try {
            String[] arr = ctx.getAssets().list("ocr/v5");
    ...
    @Override
    public OcrResult run(Bitmap bitmap) throws Exception {
        if (bitmap == null || bitmap.isRecycled()) {
            return new OcrResult("", null, 0.0f);
        }
     ...
     * @return an instance of {@link OcrResult} containing the recognized words,
     *         their spatial information, and confidence scores
     * @throws Exception if an error occurs during the OCR processing workflow
     */
    @VisibleForTesting
    OcrResult runInternal(Bitmap bitmap) throws Exception {
